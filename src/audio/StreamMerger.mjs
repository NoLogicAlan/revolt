...
export class StreamMerger extends Transform { // main stream should be piped into it
  ffmpegPath = ffmpeg;
  ffmpeg = null;
  streamTree = [];
  ...
  _transform(chunk, _enc, cb) {
    ...
    if (!this.ffmpeg.stdio[3]?.writable) { return cb(); }
  ...
  setupBaseFfmpeg() {
    ...
    this.ffmpeg.on("error", (err) => {
      console.error("[StreamMerger] Base ffmpeg error:", err.message);
      this.destroy();
    });
    this.ffmpeg.on("exit", (code, signal) => {
      if (code !== 0 && code !== null) {
        console.error("[StreamMerger] Base ffmpeg exited with code:", code, "signal:", signal);
      }
    });
  }
  spawnFfmpeg() {
  ...
  addStream(s) {
    ...
    p.on("error", (err) => {
      console.error("[StreamMerger] Child ffmpeg error:", err.message);
      // Clean up the node on error
      open.pipes.push(pipeNumber);
      open.available = true;
      const idx = open.children.findIndex(c => c == node);
      if (idx !== -1) open.children.splice(idx, 1);
    });
  }
  destroy() {
    if (this.ffmpeg) {
    ...
    this.streamTree.forEach(node => {
      const killNode = (n) => {
        n.children.forEach(c => killNode(c));
        if (n.process) n.process.kill();
      };
      killNode(node);
    });
    this.streamTree = [];
    // Clean up the Transform stream's internal state
    super.destroy();
  }
}
