...
export const run = async function(msg) {
  ...
  const Timeout = this.config.playerAFKTimeout || 10 * 60 * 1000; // 10 minutes
  ...
  msg.replyEmbed({
    ...
    const close = () => {
      ...
      unobserve();
      p.removeListener("message", playerMessageHandler);
      p.removeListener("leave", onPlayerLeave);
      m.editEmbed({
    ...
    const playerMessageHandler = () => {
      update();
    };
    const onPlayerLeave = () => {
      close();
    };
    p.on("message", playerMessageHandler);
    p.on("leave", onPlayerLeave);
